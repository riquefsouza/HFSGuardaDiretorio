...
{
	ExtensaoBO_diretorioBMP = Rotinas_imagemToPixbuf("diretorio.bmp");
	ExtensaoBO_arquivoBMP = Rotinas_imagemToPixbuf("arquivo.bmp");
    ...
    if (Rotinas_cEndsStr(sNomeDiretorio->str, ".")) {
        ...
        if (!ExtensaoBO_existeExtensao(sExtensao, listaExtensao)) {
			...
			if (pixbuf16 != NULL) {
				...
				gdk_pixbuf_unref(pixbuf16);
				gdk_pixbuf_unref(pixbuf32);
