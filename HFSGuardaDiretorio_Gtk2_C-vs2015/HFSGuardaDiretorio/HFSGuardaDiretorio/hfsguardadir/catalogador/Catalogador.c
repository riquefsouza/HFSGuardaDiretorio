...
void Catalogador_New() {
	...
	gdk_window_set_cursor(GDK_WINDOW(FrmPrincipal.frmPrincipal), gdk_cursor_new(GDK_WATCH));
	...
	gdk_window_set_cursor(GDK_WINDOW(FrmPrincipal.frmPrincipal), gdk_cursor_new(GDK_ARROW));
}
...
void Catalogador_AlterarNomeAbaAtiva(IProgressoLog pLog) {
	...
	if (Rotinas_Trim(sAba)->len > 0) {
		...
		gint pagina = gtk_notebook_current_page(GTK_NOTEBOOK(FrmPrincipal.notebook1));
...
void Catalogador_incluirNovaAba(GString* nomeAba) {
	...
	gtk_widget_set_visible(tabela, TRUE);
	gtk_widget_set_can_focus(tabela, TRUE);

	scrollTabela = gtk_scrolled_window_new(NULL, NULL);
	gtk_widget_set_name(GTK_WIDGET(scrollTabela), Rotinas_concatenar("scrollTabela_", nomeAba->str)->str);
	...
	gtk_scrolled_window_set_shadow_type(GTK_SCROLLED_WINDOW(scrollTabela), GTK_SHADOW_ETCHED_OUT);
	gtk_container_add(GTK_CONTAINER(scrollTabela), tabela);

	arvore = gtk_tree_view_new();
	gtk_widget_set_name(arvore, Rotinas_concatenar("arvore_", nomeAba->str)->str);
	gtk_widget_set_visible(arvore, TRUE);
	gtk_widget_set_can_focus(arvore, TRUE);

	scrollArvore = gtk_scrolled_window_new(NULL, NULL);
	...
	gtk_scrolled_window_set_shadow_type(GTK_SCROLLED_WINDOW(scrollArvore), GTK_SHADOW_IN);
	gtk_container_add(GTK_CONTAINER(scrollArvore), arvore);

	spPanel = gtk_hpaned_new();
	...
	gtk_container_add(GTK_CONTAINER(spPanel), scrollArvore);
	gtk_container_add(GTK_CONTAINER(spPanel), scrollTabela);

	
	GtkTreeSelection *selecaoArvore = gtk_tree_view_get_selection(GTK_TREE_VIEW(arvore));
	GtkTreeModel* modeloArvore = gtk_tree_view_get_model(GTK_TREE_VIEW(arvore));
	...
	FrmPrincipal_adicionaTabPage(GTK_NOTEBOOK(FrmPrincipal.notebook1), GTK_HPANED(spPanel), nomeAba);
}
...
GtkTreeView* Catalogador_getArvoreAtualIndice(int nIndicePagina) {
	...
	GtkWidget* scrollArvore = gtk_paned_get_child1(GTK_PANED(tabPage));
	GList* lista = gtk_container_get_children(GTK_CONTAINER(scrollArvore));
	GtkTreeView* arvore = g_list_nth_data(lista, 0);
...
GtkHPaned* Catalogador_getSplitAtual() {
	gint pagina = gtk_notebook_current_page(GTK_NOTEBOOK(FrmPrincipal.notebook1));
	GtkWidget* tabPage = gtk_notebook_get_nth_page(GTK_NOTEBOOK(FrmPrincipal.notebook1), pagina);
	GtkHPaned* split = GTK_HPANED(tabPage);
	return split;
}
...
GtkTreeView* Catalogador_getTabelaAtual() {
	gint pagina = gtk_notebook_current_page(GTK_NOTEBOOK(FrmPrincipal.notebook1));
	GtkWidget* tabPage = gtk_notebook_get_nth_page(GTK_NOTEBOOK(FrmPrincipal.notebook1), pagina);
	GtkWidget* scrollTabela = gtk_paned_get_child2(GTK_PANED(tabPage));
	GList* lista = gtk_container_get_children(GTK_CONTAINER(scrollTabela));
	GtkTreeView* tabela = g_list_nth_data(lista, 0);
	return tabela;
}
...
void Catalogador_mostrarOcultarArvore() {
	GtkHPaned* split = Catalogador_getSplitAtual();
	GtkWidget* scrollArvore = gtk_paned_get_child1(GTK_PANED(split));
...
void Catalogador_mostrarOcultarListaItensPesquisados() {
	GtkVPaned* split = GTK_VPANED(FrmPrincipal.spPesquisa);
	GtkWidget* scrollTabela = gtk_paned_get_child2(GTK_PANED(split));
...
void Catalogador_CarregarAbas() {
	...
	gtk_progress_bar_set_fraction(GTK_PROGRESS_BAR(FrmPrincipal.pb), 0);
...
void Catalogador_CarregarArquivosLista(gboolean bLista1, GtkTreeView* tabela, const GString* sCaminho) {
	...
	Tabela_carregar(tabela, bLista1, listaLocal, Catalogador.listaExtensoes, GTK_PROGRESS_BAR(FrmPrincipal.pb));
