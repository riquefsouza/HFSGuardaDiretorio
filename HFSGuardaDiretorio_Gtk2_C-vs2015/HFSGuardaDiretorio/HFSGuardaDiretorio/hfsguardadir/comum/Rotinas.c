...
GdkPixbuf* Rotinas_BlobToPixbuf(const void* blob, int tamanho) {
	GdkPixbuf *pixbuf = NULL;
	GCancellable *cancelavel = NULL;
	GError *erro = NULL;
	GInputStream *gis;

	gis = g_memory_input_stream_new_from_data(blob, tamanho, NULL);
...
GString* Rotinas_LowerCase(const GString *texto) {
	...
	return g_string_down(txt);
}

GString* Rotinas_UpperCase(const GString *texto) {
	GString *txt = g_string_new(texto->str);
	return g_string_up(txt);
}
...
GString* Rotinas_Trim(const GString *texto)
{
	...
	int tamanho = texto->len;

	if (tamanho > 0)
	{
		retorno = g_string_sized_new(tamanho);

		for (unsigned int i = 0; texto->str[i] != '\0'; i++) {
		...
		for (unsigned int i = (texto->len-1); i >= 0; i--) {
			compara = (unsigned char)texto->str[i];
			if (isspace(compara)) {
				fim = i;				
			}
...
GString* Rotinas_SubString(const GString* texto, int inicio, int tamanho) {
	GString* retorno = g_string_new("");
	gchar* temp;
	int tamtexto = strlen(texto->str);
...
ListaString Rotinas_SListLoadFromFile(const GString* sNomeArquivo) {
	...
	if (info != NULL) {
		...
		texto = Rotinas_cNewStr(tamArquivo);
...
long Rotinas_BlobLoadFromFile(const GString* sNomeArquivo, void** blob) {
	long ret = 0;
	FILE* fp;
	errno_t erro;
	//	long tamanho;
...
Arquivo Rotinas_FileInfoToArquivo(const GString *sCaminho) {
	...
	if (garquivo != NULL) {
		...
		g_file_info_get_modification_time(info, &tempoGArquivo);
		...
		arq->modificado = g_date_time_new_from_timeval_local(&tempoGArquivo);
...
void Rotinas_testar() {
	...
	void *blob = NULL;
	GString *sImgArq = g_string_new("C:/andarDIR/HFSGuardaDiretorio_Gtk2_C-vs2015/HFSGuardaDiretorio/Debug/imagens/diretorio.bmp");
	...
	if (pixbuf != NULL) {
		...
		if (Rotinas_PixbufToArquivo(pixbuf, "C:\\Temp\\blob\\dir32.bmp", Rotinas_EXTENSAO_BMP)) {
	...
	if (pixbuf != NULL){
		...
		if (Rotinas_PixbufToArquivo(pixbuf, "C:\\Temp\\blob\\dir.bmp", Rotinas_EXTENSAO_BMP)) {
			OutputDebugString("Rotinas_PixbufToArquivo OK");
		}
	}
	...
	if (pixbuf != NULL) {
		...
		if (Rotinas_PixbufToArquivo(pixbuf, "C:\\Temp\\blob\\dir.gif", Rotinas_EXTENSAO_GIF)) {
			OutputDebugString("Rotinas_PixbufToArquivo OK");
		}
	}
	gdk_pixbuf_unref(pixbuf);
	...
	if (pixbuf != NULL) {
		...
		if (Rotinas_PixbufToArquivo(pixbuf, "C:\\Temp\\blob\\dir.jpg", Rotinas_EXTENSAO_JPEG)) {
			OutputDebugString("Rotinas_PixbufToArquivo OK");
		}
	}
	gdk_pixbuf_unref(pixbuf);
	...
	*/
	...
	g_string_printf(saida, "sDir: [%s]\n", sDir->str); OutputDebugString(saida->str);
	g_string_printf(saida, "AppDirPath: %s\n", sApp->str); OutputDebugString(saida->str);
	g_string_printf(saida, "FileExists: %s\n", (Rotinas_FileExists(sApp) ? "true" : "false")); OutputDebugString(saida->str);
	g_string_printf(saida, "DirectoryExistsLinux: %s\n", (Rotinas_DirectoryExistsLinux(sApp) ? "true" : "false")); OutputDebugString(saida->str);
	g_string_printf(saida, "ExtractFilePath: %s\n", Rotinas_ExtractFilePath(sApp)->str); OutputDebugString(saida->str);
	g_string_printf(saida, "ExtractFileName: %s\n", Rotinas_ExtractFileName(sApp)->str); OutputDebugString(saida->str);

	g_string_printf(saida, "Length(): %d\n", sDir->len); OutputDebugString(saida->str);
	GString *sTrim = Rotinas_Trim(sDir);
	g_string_printf(saida, "Trim(): [%s]\n", sTrim->str); OutputDebugString(saida->str);
	g_string_printf(saida, "tamanho Trim(): %d\n", sTrim->len); OutputDebugString(saida->str);
	g_string_printf(saida, "LowerCase(): %s\n", Rotinas_LowerCase(sDir)->str); OutputDebugString(saida->str);
	g_string_printf(saida, "UpperCase(): %s\n", Rotinas_UpperCase(sDir)->str); OutputDebugString(saida->str);
	g_string_printf(saida, "Pos('.'): %d\n", Rotinas_Pos(g_string_new("."), sDir)); OutputDebugString(saida->str);
	g_string_printf(saida, "LastDelimiter('.'): %d\n", Rotinas_LastDelimiter(g_string_new("."), sDir)); OutputDebugString(saida->str);
	g_string_printf(saida, "QuotedStr(): %s\n", Rotinas_cQuotedStr(sDir->str)); OutputDebugString(saida->str);
	g_string_printf(saida, "IntToStr(245789): %s\n", Rotinas_IntToStr(245789)->str); OutputDebugString(saida->str);
	g_string_printf(saida, "StrToInt() --> 245789: %d\n", Rotinas_StrToInt(g_string_new("245789"))); OutputDebugString(saida->str);
	g_string_printf(saida, "StrToLong() --> 4200155136: %llu\n", Rotinas_StrToLong(g_string_new("4200155136"))); OutputDebugString(saida->str);
	g_string_printf(saida, "DoubleToStr(15.557878): %s\n", Rotinas_DoubleToStr(15.557878)->str); OutputDebugString(saida->str);

	g_string_printf(saida, "SubString(): %s\n", Rotinas_SubString(sDir, 8, 6)->str); OutputDebugString(saida->str);
	g_string_printf(saida, "cStringReplaceAll(): [%s]\n", Rotinas_cReplaceAll(sDir->str, "\\", "@")); OutputDebugString(saida->str);
	g_string_printf(saida, "trocaSeparador(): [%s]\n", Rotinas_trocaSeparador(sDir)->str); OutputDebugString(saida->str);
	g_string_printf(saida, "testaNull(NULL): %s\n", Rotinas_testaNull(NULL)->str); OutputDebugString(saida->str);

	g_string_printf(saida, "StartsStr(DVD, JOGOS2_DVDS/TESTE): %s\n", (Rotinas_StartsStr(g_string_new("DVD"), g_string_new("JOGOS2_DVDS/TESTE")) ? "true" : "false")); OutputDebugString(saida->str);
	g_string_printf(saida, "StartsStr(DVD, DVDSJOGOS2_/TESTE): %s\n", (Rotinas_StartsStr(g_string_new("DVD"), g_string_new("DVDSJOGOS2_/TESTE")) ? "true" : "false")); OutputDebugString(saida->str);

	g_string_printf(saida, "ContainsStr(JOGOS2_DVDS/TESTE): %s\n", (Rotinas_ContainsStr(g_string_new("DVDSJOGOS2_/TESTE"), g_string_new("DVDSJOGOS2_/TESTE")) ? "true" : "false")); OutputDebugString(saida->str);
	g_string_printf(saida, "EndsStr(TESTE, JOGOS2_DVDS/TESTE): %s\n", (Rotinas_EndsStr(g_string_new("TESTE"), g_string_new("DVDSJOGOS2_/TESTE")) ? "true" : "false")); OutputDebugString(saida->str);

	g_string_printf(saida, "formataDate(): %s\n", Rotinas_formataDate(Rotinas_FORMATO_DATAHORA, g_date_time_new_now_local())->str); OutputDebugString(saida->str);
	GDateTime* dt = Rotinas_StringToDate(g_string_new("03/02/2017 15:23:32"));		
	g_string_printf(saida, "StringToDate(03/02/2017 15:23:32): %s\n", g_date_time_format(dt, Rotinas_FORMATO_DATAHORA)); OutputDebugString(saida->str);

	g_string_printf(saida, "FormatLong(4200155136): %s\n", Rotinas_ULongToStr(4200155136)->str); OutputDebugString(saida->str);
	g_string_printf(saida, "FormatDouble(15.544878): %s\n", Rotinas_DoubleToStr(15.544878)->str); OutputDebugString(saida->str);
	g_string_printf(saida, "FormatDouble(15.547878): %s\n", Rotinas_DoubleToStr(15.547878)->str); OutputDebugString(saida->str);
	g_string_printf(saida, "FormatDouble(15.557878): %s\n", Rotinas_DoubleToStr(15.557878)->str); OutputDebugString(saida->str);
	g_string_printf(saida, "FormatDouble(15.567878): %s\n", Rotinas_DoubleToStr(15.567878)->str); OutputDebugString(saida->str);

	g_string_printf(saida, "MontaTamanho() --> 3.91 GB: %s\n", Rotinas_MontaTamanho(4200155136)->str); OutputDebugString(saida->str);
	g_string_printf(saida, "MontaTamanho() --> 3.50 GB: %s\n", Rotinas_MontaTamanho(3757244416)->str); OutputDebugString(saida->str);
	g_string_printf(saida, "MontaTamanho() --> 2.29 GB: %s\n", Rotinas_MontaTamanho(2455207936)->str); OutputDebugString(saida->str);
	g_string_printf(saida, "MontaTamanho() --> 888.18 PB: %s\n", Rotinas_MontaTamanhoBig(999999999999999999.9999)->str); OutputDebugString(saida->str);
