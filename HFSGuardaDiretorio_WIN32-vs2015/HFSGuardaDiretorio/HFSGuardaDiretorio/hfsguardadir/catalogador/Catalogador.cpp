...
void Catalogador_AddItemArvore(Arvore arvore, Diretorio diretorio, int Nivel,
    ...
    if (Nivel == 0) {
		item1 = TreeView_InsertItem(arvore, &Arvore_item(diretorio.arquivo.nome.wstr, NULL, TVI_ROOT, Nivel));
	...
	for (local = listaFilhos; local != NULL; local = local->proximo) {
		...
		item2 = TreeView_InsertItem(arvore, &Arvore_item(filho.arquivo.nome.wstr, item1, TVI_LAST, Nivel));
