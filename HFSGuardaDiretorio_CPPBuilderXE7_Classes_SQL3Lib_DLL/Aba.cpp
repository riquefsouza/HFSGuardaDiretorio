...
void TAba::carregarAba(std::vector<TAba> *listaAba, TProgressoLog progressoLog) {
	...
	dm->tabelaAbas=dm->conexao->GetTable(dm->SQLAbas.c_str());
	...
	if (total > 0) {
		...
		for (int i = 0; i < total; i++) {
			...
			*/
			...
			aba.codigo = dm->tabelaAbas->FieldAsInteger(
				dm->tabelaAbas->FieldIndex("cod"));  //0
			aba.nome = dm->tabelaAbas->FieldAsString(
				dm->tabelaAbas->FieldIndex("nome")).c_str(); //1
