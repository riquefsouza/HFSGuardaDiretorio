...
uses
  ...
  uDataModule, Data.SqlExpr, Data.DB,
...
function atribuiDiretorio(nAba, nCodDirRaiz: integer;
  ...
  listaLocal: TList<TDiretorio>; arquivo: TArquivo; var nOrdem: integer)
  : TDiretorio;
...
procedure excluirListaDiretorio(listaDiretorio: TList<TDiretorio>; aba: TAba;
  sCaminho: String);
...
function itensFilhos(lista_pai: TList<TDiretorio>;
  aba, codDirPai, codigo: integer): TList<TDiretorio>;
...
var
  sSQL: string;
begin
  sSQL := 'create table IF NOT EXISTS Diretorios(' + 'aba integer not null,' +
    'cod integer not null,' + 'ordem integer not null,' +
    'nome varchar(255) not null,' + 'tam numeric not null,' +
    'tipo char(1) not null,' + 'modificado date not null,' +
    'atributos varchar(20) not null,' + 'coddirpai integer not null,' +
    'caminho varchar(255) not null,' + 'primary key (aba, cod, ordem))';
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        modificado := StrToDateTime(consultaDir.Fields.FieldByNumber(8)
          .AsString);
        ...
        if tipo.codigo = 'D' then
          ...
          tipo.nome := 'Arquivo';
        modificadoFormatado := FormatDateTime('dd/mm/yyyy hh:nn:ss',
          modificado);
      end;
...

{
  ...
  listaDiretorio: TListaDiretorio; pb: TProgressBar);
  var
  diretorio: TDiretorio;
  i, total: integer;
...
begin
  ...
  begin
    if (lista_pai.items[i].aba.codigo = aba) then
    begin
      if (lista_pai.items[i].codDirPai = codDirPai) then
      begin
        if (lista_pai.items[i].codigo = codigo) then
        begin
...
begin
  sAtributos := '';
  if (nAtributos and System.SysUtils.faReadOnly > 0) then
    sAtributos := sAtributos + '[ROL]';
  if (nAtributos and System.SysUtils.faHidden > 0) then
    sAtributos := sAtributos + '[HID]';
  if (nAtributos and System.SysUtils.faSysFile > 0) then
    sAtributos := sAtributos + '[SYS]';
  // if (nAtributos and System.SysUtils.faVolumeID > 0) then sAtributos:=sAtributos+'[VOL]';
  if (nAtributos and System.SysUtils.faDirectory > 0) then
    sAtributos := sAtributos + '[DIR]';
  if (nAtributos and System.SysUtils.faArchive > 0) then
...
begin
  ...
  begin
    ...
    begin
      if diretorio.caminho = sDiretorio then
      begin
...
begin
  ...
  if (Length(sCaminho) > 0) and (Length(sNomeDirRaiz) > 0) then
  begin
    if AnsiStartsStr(sNomeDirRaiz, sCaminho) then
    begin
  ...
  begin
    ...
    begin
      caminho := sCaminho;
      caminhoPai := copy(caminho, 1, LastDelimiter(BARRA_INVERTIDA,
        caminho) - 1);
    end;
...
var
  diretorio: TDiretorio;
begin
  if FileExists(sDiretorio) then
  begin
    diretorio := atribuiDiretorio(nAba, nCodDirRaiz, sNomeDirRaiz, sDiretorio,
      False, listaLocal, searchRecToArquivo(SearchRec), nOrdem);
    listaLocal.Add(diretorio);
    log.Append(sDiretorio);
  end
  else if DirectoryExists(sDiretorio) then
  begin
    if (SearchRec.Name <> '.') and (SearchRec.Name <> '..') then
    begin
      diretorio := atribuiDiretorio(nAba, nCodDirRaiz, sNomeDirRaiz, sDiretorio,
        true, listaLocal, searchRecToArquivo(SearchRec), nOrdem);
      listaLocal.Add(diretorio);
      log.Append(sDiretorio);

      ImportarDiretorio(nAba, nCodDirRaiz, sNomeDirRaiz, sDiretorio, listaLocal,
        nOrdem, log);
    end;
  end;
end;
...
begin
  ...
  if FindFirst(sCaminho, faAnyFile, SearchRec) = 0 then
  begin

    sCaminho := sDiretorio + sSeparador + SearchRec.Name;
    atribuiListaDiretorio(nAba, nCodDirRaiz, sNomeDirRaiz, sCaminho, listaLocal,
      SearchRec, nOrdem, log);
...
begin
  ...
  begin
    ...
    sXML := sXML + sTAB2 + '<modificado>' + modificadoFormatado +
      '</modificado>' + sCR;
    sXML := sXML + sTAB2 + '<atributos>' + atributos + '</atributos>' + sCR;
    sXML := sXML + sTAB2 + '<codDirPai>' + IntToStr(codDirPai) +
      '</codDirPai>' + sCR;
...
begin
  ...
  begin
    ...
    sHTML := sHTML + sTAB2 + '<td>' + StringReplace(tamanhoFormatado, ' ',
      '&nbsp;', [rfReplaceAll, rfIgnoreCase]) + '</td>' + sCR;
    sHTML := sHTML + sTAB2 + '<td>' + tipo.nome + '</td>' + sCR;
    sHTML := sHTML + sTAB2 + '<td>' + StringReplace(modificadoFormatado, ' ',
      '&nbsp;', [rfReplaceAll, rfIgnoreCase]) + '</td>' + sCR;
    ...
    case tipo of
      ...
      teHTML:
        begin
          ...
          sTexto := sTexto +
            '<table border="1" cellpadding="5" cellspacing="0">' + sCR;
...
procedure incluirDiretorio(diretorio: TDiretorio);
begin
  if dm.cdsDiretorios.Active then
  begin
    dm.cdsDiretorios.ReadOnly := False;
    dm.cdsDiretorios.Append;
    {
      dm.cdsDiretoriosaba.Value:=diretorio.aba.codigo;
      dm.cdsDiretorioscod.Value:=diretorio.codigo;
      dm.cdsDiretoriosordem.Value:=diretorio.ordem;
      dm.cdsDiretoriosnome.Value:=diretorio.nome;
      dm.cdsDiretoriostam.Value:=diretorio.tamanho;
      dm.cdsDiretoriostipo.Value:=diretorio.tipo.codigo;
      dm.cdsDiretoriosmodificado.Value:=FormatDateTime('dd/mm/yyyy hh:nn:ss',
      diretorio.modificado);
      dm.cdsDiretoriosatributos.Value:=diretorio.atributos;
      dm.cdsDiretorioscoddirpai.Value:=diretorio.coddirpai;
      dm.cdsDiretorioscaminho.Value:=diretorio.caminho;
    }
...
begin
  ...
  if (nArquivo = 3) and (sTagInicio <> '<nomeAba>') and (sTagFim <> '</nomeAba>')
  then
  begin
    // diretorio.aba.nome := sValor;
    result := -1;
  end;
  if (nArquivo = 4) and (sTagInicio = '<codigo>') and (sTagFim = '</codigo>')
  then
  begin
  ...
  if (nArquivo = 7) and (sTagInicio = '<tamanho>') and (sTagFim = '</tamanho>')
  then
  begin
  ...
  if (nArquivo = 9) and (sTagInicio = '<modificado>') and
    (sTagFim = '</modificado>') then
  begin
    diretorio.modificado := StrToDateTime(sValor);
  end;
  if (nArquivo = 10) and (sTagInicio = '<atributos>') and
    (sTagFim = '</atributos>') then
  begin
    diretorio.atributos := sValor;
  end;
  if (nArquivo = 11) and (sTagInicio = '<codDirPai>') and
    (sTagFim = '</codDirPai>') then
  begin
    diretorio.codDirPai := StrToInt(sValor);
  end;
  if (nArquivo = 12) and (sTagInicio = '<caminho>') and (sTagFim = '</caminho>')
  then
  begin
  ...
  try
    ...
    begin
      ...
      begin
        ...
        else if nArquivo = 13 then
        begin
          nArquivo := 1;
          diretorio.aba.codigo := aba.codigo;
          listaDiretorio.Add(diretorio);

          if verificaCodDir(aba.codigo, diretorio.caminho, listaDirPai) then
          begin
            result := -2;
            break;
          end;
...
begin
  ...
  sCaminho := sDiretorio + sSeparador + '*.*';
  if FindFirst(sCaminho, faDirectory, SearchRec) = 0 then
  begin
...
begin
  ...
  sCaminho := sDiretorio + sSeparador + '*.*';
  if FindFirst(sCaminho, faArchive, SearchRec) = 0 then
  begin
...
begin
  ...
  begin
    ...
    else if DirectoryExists(sCaminho) then
    begin
      if (SearchRec.Name <> '.') and (SearchRec.Name <> '..') then
      begin
