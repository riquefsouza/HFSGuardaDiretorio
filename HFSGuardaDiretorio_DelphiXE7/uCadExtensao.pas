...
begin
  ...
  begin
    ...
    begin
      ...
      if excluirExtensao(FrmPrincipal.listaExtensoes, extensao.codigo) then
      begin
...
begin
  ...
  begin
    if MessageDlg('Tem Certeza, que você deseja excluir todas as extensões?',
      mtConfirmation, [mbYes, mbNo], 0) = mrYes then
    begin
      if excluirTodasExtensoes(FrmPrincipal.listaExtensoes) then
      begin
        CarregarExtensoesNaGrid;
  ...
  begin
    ...
    try
      if SalvarExtensao(FrmPrincipal.listaExtensoes,
        ExtractFileName(OpenDialog1.FileName), OpenDialog1.FileName, log) then
      begin
...
procedure TFrmCadExtensao.sgExtensaoDrawCell(Sender: TObject;
...
begin
  ...
  for i := 0 to FrmPrincipal.listaExtensoes.Count - 1 do
  begin
