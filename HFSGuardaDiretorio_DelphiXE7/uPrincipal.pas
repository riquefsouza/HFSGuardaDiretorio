...
uses
  ...
  System.Variants, System.UITypes, System.Classes, Vcl.Graphics,
  System.Generics.Collections, Vcl.Controls, Vcl.Forms, Vcl.Dialogs,
  Vcl.Menus, Vcl.ComCtrls, Vcl.ExtCtrls, Vcl.StdCtrls, Vcl.Buttons,
  uRotinas, Vcl.ImgList, Data.DB, uDataModule, uEscolherDir, uImportar,
  uSobre, uCadExtensao, uSplash, uAba, uDiretorio, uExtensao;
...
type
  TFrmPrincipal = class(TForm)
    ...
    HTML1: TMenuItem;
    Viso1: TMenuItem;
    MostrarOcultarArvoreDirAbaAtiva1: TMenuItem;
    ...
    procedure MostrarOcultarArvoreDirAbaAtiva1Click(Sender: TObject);
  ...
  private
    ...
    function PesquisarArvoreDiretorio(tvLocal: TTreeView; sCaminhoPai: string)
      : TTreeNode;
  ...
  public
    ...
    function LerArvoreDiretorio(MyTreeNode1: TTreeNode; barraStatus: TStatusBar)
      : TTreeNode;
...
uses uImportarDiretorio, uCompararDiretorio, uInfoDiretorio;
...
var
  aba: TAba;
begin
  if InputQuery('Digite o nome da nova Aba?', 'Nome da Aba: ', aba.nome) then
  begin
    aba.codigo := retMaxCodAba(listaAbas);
...
begin
  ...
  listaFilhos := itensFilhos(listaDiretorioFilho, diretorio.aba.codigo,
...
begin
  result := TSplitter(pageControl.Pages[pageControl.ActivePageIndex]
    .Controls[0])
end;

function TFrmPrincipal.getAbaAtual: TAba;
begin
  result := pegaAbaPorOrdem(listaAbas, pageControl.ActivePageIndex + 1);
end;
...
begin
  aba := getAbaAtual;
  if InputQuery('Digite o novo nome da Aba ativa?', 'Nome da Aba: ', aba.nome)
  then
  begin
    aba.nome := Copy(aba.nome, 1, 10);
    alterarAba(aba);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        excluirDiretorio(getAbaAtual, barraStatus.Panels[1].Text);
...
begin
  carregarAba(listaAbas, pLog);
  if bExtensao then begin
    carregarExtensao(listaExtensoes, pLog);
    carregarExtensoes(listaExtensoes, ImageList1, ImageList2);
  end;
  if bDiretorio then begin
    carregarDiretorio(dm.consultaDirPai, listaDiretorioPai, pLog);
    carregarDiretorio(dm.consultaDirFilho, listaDiretorioFilho, pLog);
    carregarDiretorio(dm.consultaArquivo, listaArquivos, pLog);
  end;
end;

procedure TFrmPrincipal.ExcluirDados(aba: TAba; sCaminho: String);
begin
  excluirListaDiretorio(listaDiretorioPai, aba, sCaminho);
  excluirListaDiretorio(listaDiretorioFilho, aba, sCaminho);
  excluirListaDiretorio(listaArquivos, aba, sCaminho);
end;
...
function TFrmPrincipal.verificaNomeDiretorio(sCaminho: string;
  var sRotuloRaiz: string): integer;
begin
  sRotuloRaiz := removerDrive(sCaminho);
  if Length(sRotuloRaiz) = 0 then
  begin
    if InputQuery('Este diretório não possui um nome!',
      'Digite o nome do diretório: ', sRotuloRaiz) then
    begin
  ...
  else if Pos(BARRA_INVERTIDA, sRotuloRaiz) > 0 then
  begin
    sRotuloRaiz := Copy(sRotuloRaiz, LastDelimiter(BARRA_INVERTIDA, sRotuloRaiz)
      + 1, Length(sRotuloRaiz));
    result := 2;
  end
  else
    result := 3;
...
begin
  ...
  if FrmEscolherDir.ShowModal() = mrOk then
  begin
    ...
    try
      ...
      if ImportarDiretorios(listaCaminho, bSubDiretorios) then
      begin
        FinalizaImportacao;
      end;
    finally
...
begin
  if pageControl.ActivePageIndex = 0 then
    result := TListView(pageControl.Pages[pageControl.ActivePageIndex]
      .Controls[1])
  else
    result := TListView(pageControl.Pages[pageControl.ActivePageIndex]
      .Controls[2]);
...
begin
  ...
  case colOrdem of
    ...
    2:  Compare := tag * CompareText(Item1.subitems[colOrdem-1],
      Item2.subitems[colOrdem-1]);
    3:  Compare := tag * CompareTextAsDateTime(Item1.subitems[colOrdem-1],
      Item2.subitems[colOrdem-1]);
...
begin
  ...
  begin
    ...
    begin
      ...
      nAltura :=
        round(abs(lvLocal.Font.Height * lvLocal.Font.PixelsPerInch) / 72) + 2;
...
procedure TFrmPrincipal.MostrarOcultarListaItensPesquisados1Click
  (Sender: TObject);
begin
  if lvPesquisa.Visible = False then
  begin
...
procedure TFrmPrincipal.MostrarOcultarArvoreDirAbaAtiva1Click(Sender: TObject);
var
  tvLocal: TTreeView;
  spLocal: TSplitter;
begin
...
procedure TFrmPrincipal.DuploCliqueLista(lvLocal: TListView;
  tvLocal: TTreeView);
var
  sCaminho: string;
begin
  ...
  begin
    if lvLocal.Selected.SubItems.Strings[1] = 'Diretório' then
    begin
      ...
      if Copy(sCaminho, Length(sCaminho), 1) = BARRA_INVERTIDA then
        ...
        sCaminho := barraStatus.Panels[1].Text + BARRA_INVERTIDA +
...
begin
  ...
  begin
    if Copy(NodeAnterior.Text, Length(NodeAnterior.Text), 1) = BARRA_INVERTIDA
    then
      sSep := ''
    else
...
begin
  ...
  begin
    ...
    list1.ImageIndex := indiceExtensao(listaExtensoes, diretorio.nome);
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if (diretorio.caminhoPai = sPaiCaminho) then
        begin
          if AnsiStartsStr(sCaminho, diretorio.caminho) then
          begin
...
begin
  ...
  begin
    ...
    begin
      if (diretorio.tipo.codigo <> 'D') then
      begin
        if Length(Trim(sCaminho)) > 0 then
        begin
          if AnsiStartsStr(sCaminho, diretorio.caminho) then
          begin
            conta := conta + 1;
  ...
  barraStatus.Panels[0].Text := 'Quantidade Total: ' +
    FormatFloat('0000', conta) + ', Tamanho Total: ' + MontaTamanhoBig(soma);
...

{
  ...
  ', Tamanho Total: '+MontaTamanhoBig(soma);
...
var
  proximoNode, diretorioNode: TTreeNode;
  slTemp: TStringList;
  nPos: integer;
begin
  Screen.Cursor := crHourGlass;
  ...
  slTemp.Text := StringReplace(sCaminhoPai, BARRA_INVERTIDA, #13#10,
    [rfReplaceAll, rfIgnoreCase]);

  proximoNode := tvLocal.Items.Item[0];
  diretorioNode := nil;
  nPos := 0;
  while proximoNode <> nil do
  begin

    if slTemp.Strings[nPos] = proximoNode.Text then
    begin
      diretorioNode := proximoNode;

      if nPos = (slTemp.Count - 1) then
      begin
        break;
      end;
      nPos := nPos + 1;
...
begin
  ...
  begin
    ...
    begin
      Screen.Cursor := crHourGlass;
      exportarDiretorio(tipo, getAbaAtual, sdExportar.FileName, ProgressoLog);
      Screen.Cursor := crDefault;
...
begin
  ...
  begin
    ...
    begin
      Screen.Cursor := crHourGlass;
      nResultado := importarDiretorioViaXML(getAbaAtual, odImportar.FileName,
...
begin
  ...
  nCodDirRaiz := retMaxCodDir(nAba, listaDiretorioPai);
  ...
  begin
    ...
    begin
      ...
      begin
        sCaminhoSemDrive := removerDrive(sCaminho);
  ...
  begin
    if not verificaCodDir(importar.aba, importar.rotuloRaiz, listaDiretorioPai)
    then
    begin
      FrmImportarDiretorio.ShowModal;
      result := true;
