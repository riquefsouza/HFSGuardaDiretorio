...
uses
  ...
  Vcl.ComCtrls, Vcl.StdCtrls, Vcl.ExtCtrls, uDiretorio, uAba, uRotinas;
...
type
  ...
  private
    ...
    listaCompara: TList<TDiretorio>;
    procedure LimparComparacao;
    procedure ProgressoLog(progresso: TProgresso);
    function SQLCompara(aba1, aba2: TAba; caminho1, caminho2: String): String;
...
uses uPrincipal, uImportar, uDataModule;

function TFrmCompararDiretorio.SQLCompara(aba1, aba2: TAba;
...
begin
  sSQL := SQL_CONSULTA_ARQUIVO + ' WHERE aba=' + IntToStr(aba1.codigo) +
    ...
    ' SELECT nome ' + ' FROM Diretorios ' + ' WHERE aba=' +
    IntToStr(aba2.codigo) + ' AND caminho LIKE ' + QuotedStr(caminho2 + '%') +
    ')' + ' ORDER BY 1, 2, 3';
  result := sSQL;
end;
...
var
  ...
  aba2: TAba;
begin
  aba1 := pegaAbaPorOrdem(FrmPrincipal.listaAbas, cmbAba1.ItemIndex + 1);
  aba2 := pegaAbaPorOrdem(FrmPrincipal.listaAbas, cmbAba2.ItemIndex + 1);
  dm.consultaAuxiliar.SQL.Text := SQLCompara(aba1, aba2, sCaminhoDir1,
    sCaminhoDir2);
  carregarDiretorio(dm.consultaAuxiliar, listaCompara, ProgressoLog);
  ...
  begin
    ...
    MessageDlg('Nenhuma diferença encontrada!', mtInformation, [mbOk], 0, mbOk);
end;
...
begin
  ...
  if tvDiretorio1.Selected <> nil then
  begin
    if tvDiretorio1.Selected.Selected then
    begin
      FrmPrincipal.LerArvoreDiretorio(tvDiretorio1.Selected, barraStatus);
      sCaminhoDir1 := barraStatus.Panels[1].Text;

      if tvDiretorio2.Selected <> nil then
      begin
        if tvDiretorio2.Selected.Selected then
        begin
  ...
  if bSelecionado then
    ...
    MessageDlg('Diretórios não selecionados!', mtInformation, [mbOk], 0, mbOk);
end;
...
begin
  ...
  begin
    ...
    try
      ...
      for i := 0 to listaCompara.Count - 1 do
      begin
        diretorio := listaCompara.Items[i];
        listaLocal.Append(diretorio.caminho);
      end;
      listaLocal.SaveToFile(sLog);
      MessageDlg('Log salvo no mesmo diretório do sistema!', mtInformation,
        [mbOk], 0);
    finally
      listaLocal.Free;
    end;
...
begin
  ...
  aba := pegaAbaPorOrdem(FrmPrincipal.listaAbas, cmbAba1.ItemIndex + 1);
...
begin
  ...
  aba := pegaAbaPorOrdem(FrmPrincipal.listaAbas, cmbAba2.ItemIndex + 1);
...
begin
  listaCompara := TList<TDiretorio>.Create;
end;
...
begin
  ...
  FrmPrincipal.CarregarArvore(tvDiretorio1,
    pegaAbaPorOrdem(FrmPrincipal.listaAbas, cmbAba1.ItemIndex + 1));
  FrmPrincipal.CarregarArvore(tvDiretorio2,
    pegaAbaPorOrdem(FrmPrincipal.listaAbas, cmbAba2.ItemIndex + 1));
end;
