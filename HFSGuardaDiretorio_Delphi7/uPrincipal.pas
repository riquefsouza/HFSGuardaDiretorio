...
uses
  Windows, Messages, SysUtils, Variants, StrUtils,
  Classes, Graphics, uListaDiretorio,
  Controls, Forms, Dialogs, Menus, ComCtrls, ExtCtrls,
  StdCtrls, Buttons, uRotinas, ImgList, DB,
  uDataModule, uEscolherDir, uImportar, uSobre, uCadExtensao, uSplash, uAba,
  uDiretorio, uExtensao, uListaExtensao, uListaAba, uListaImportar;
...
type
  TFrmPrincipal = class(TForm)
    ...
    procedure lvPesquisaCompare(Sender: TObject; Item1, Item2: TListItem;
      Data: Integer; var Compare: Integer);
    procedure tvFixaCollapsed(Sender: TObject; Node: TTreeNode);
    procedure tvFixaExpanded(Sender: TObject; Node: TTreeNode);
  private
...
begin
  ...
  tv.OnCollapsed:= tvFixaCollapsed;
  tv.OnExpanded:= tvFixaExpanded;
...
begin
  if atribuirParamsConexao then begin
    screen.Cursor := crHourGlass;
...
begin
  ...
  if bSubDiretorio then begin
    ...
    if not verificaCodDir(importar.aba, importar.rotuloRaiz,
...
procedure TFrmPrincipal.lvFixaColumnClick(Sender: TObject;
  Column: TListColumn);
begin
  listaColunaClick(Sender, Column);
end;

procedure TFrmPrincipal.lvFixaCompare(Sender: TObject; Item1,
...
procedure TFrmPrincipal.tvFixaCollapsed(Sender: TObject; Node: TTreeNode);
begin
  Node.ImageIndex:=0;
end;

procedure TFrmPrincipal.tvFixaExpanded(Sender: TObject; Node: TTreeNode);
begin
  Node.ImageIndex:=1;
end;
