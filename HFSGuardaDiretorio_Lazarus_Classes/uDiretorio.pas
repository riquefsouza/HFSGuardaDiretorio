...
var
  diretorio: TDiretorio;
begin
    if FileExistsUTF8(sDiretorio) then begin
    ...
    end else if DirectoryExistsUTF8(sDiretorio) then begin
...
begin
  ...
  if FindFirstUTF8(sCaminho,faAnyFile,SearchRec) = 0 then begin
    ...
    while FindNextUTF8(SearchRec) = 0 do begin
      sCaminho:=sDiretorio+sSeparador+SearchRec.Name;
      atribuiListaDiretorio(nAba, nCodDirRaiz, sNomeDirRaiz, sCaminho,
        listaLocal, SearchRec, nOrdem, log);
    end;
...
begin
  ...
  sCaminho:=sDiretorio+sSeparador+'*.*';
  if FindFirstUTF8(sCaminho,faDirectory,SearchRec) = 0 then begin

    while FindNextUTF8(SearchRec) = 0 do begin
      sCaminho:=sDiretorio+sSeparador+SearchRec.Name;

      if SearchRec.Attr=faDirectory then begin
        if DirectoryExistsUTF8(sCaminho) then begin
          if (SearchRec.Name<>'.') and (SearchRec.Name<>'..') then begin
...
begin
  ...
  sCaminho:=sDiretorio+sSeparador+'*.*';
  if FindFirstUTF8(sCaminho,faArchive,SearchRec) = 0 then begin

    while FindNextUTF8(SearchRec) = 0 do begin
      sCaminho:=sDiretorio+sSeparador+SearchRec.Name;

      if SearchRec.Attr=faArchive then begin
        if FileExistsUTF8(sCaminho) then begin
          if (SearchRec.Name<>'.') and (SearchRec.Name<>'..') then begin
              listaLocal.Add(sCaminho);
          end;
        end;
...
begin
  ...
  if FindFirstUTF8(sCaminho,faAnyFile,SearchRec) = 0 then begin
    ...
    if FileExistsUTF8(sCaminho) then begin
      listaLocal.Add(sCaminho);
    end else if DirectoryExistsUTF8(sCaminho) then begin
      if (SearchRec.Name<>'.') and (SearchRec.Name<>'..') then begin
