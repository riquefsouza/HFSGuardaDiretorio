          ...
          begin
            ...
            //get data types
              ...
              begin
                ...
                sDeclaredColType := DeclaredColType;
                sDeclaredColType := AnsiUpperCase(sDeclaredColType);

                if DeclaredColType = nil then
                  thisColType^ := Sqlite3_ColumnType(stmt, i) //use the actual column type instead
                //seems to be needed for last_insert_rowid
                else
                  if (sDeclaredColType = 'INTEGER') or (sDeclaredColType = 'BOOLEAN') then
                    thisColType^ := dtInt
                  else
                    if (sDeclaredColType = 'NUMERIC') or
                      (sDeclaredColType = 'FLOAT') or
                      (sDeclaredColType = 'DOUBLE') or
                      (sDeclaredColType = 'REAL') then
                      thisColType^ := dtNumeric
                    else
                      if sDeclaredColType = 'BLOB' then
                        thisColType^ := dtBlob
                      else
                        thisColType^ := dtStr;
