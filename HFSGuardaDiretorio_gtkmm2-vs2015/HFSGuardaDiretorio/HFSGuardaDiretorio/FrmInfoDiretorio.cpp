...
FrmInfoDiretorio::FrmInfoDiretorio()
	...
	colunaInfoDescricao->set_sort_column_id(1);
	
	Gtk::CellRendererText rtexto;
	Pango::FontDescription pfd("Bold");
	Glib::PropertyProxy<Pango::FontDescription> pc = rtexto.property_font_desc();
	pc.operator Pango::FontDescription().operator=(pfd);
	...
	colunaInfoItem->set_sort_column_id(1);
	...
	dialog_vbox3 = manage(new class Gtk::VBox(false, 0));
	dialog_vbox3->set_name("dialog_vbox3");
	dialog_vbox3->set_visible(true);
	dialog_vbox3->set_can_focus(false);
	dialog_vbox3->set_spacing(2);
	dialog_vbox3->pack_start(*vbox3, true, true, 0);
	dialog_vbox3->pack_start(*dialog_action_area3, false, true, 0);
	
	this->set_name("frmInfoDiretorio");
	this->set_size_request(385, 329);
	...
	this->get_vbox()->add(*dialog_vbox3);
