...
void Catalogador::iniciarSistema() {
	Aba aba;
	Glib::ustring sBanco = Rotinas::AppDirName();
	sBanco += "GuardaDir.db";
...
void Catalogador::ExcluirDados(Aba aba, const Glib::ustring &sCaminho) {
...
void Catalogador::IncluirNovaAba() {
	...
	Glib::ustring sAba = Dialogo::entrada(frmPrincipal,
...
void Catalogador::AlterarNomeAbaAtiva(IProgressoLog pLog) {
	Aba aba = getAbaAtual();
	Glib::ustring sAba = Dialogo::entrada2(frmPrincipal,
...
void Catalogador::incluirNovaAba(const Glib::ustring &nomeAba) {
	Gtk::HPaned *spPanel;
	Arvore *arvore;
	Tabela *tabela;
...
void Catalogador::CarregarArquivosLista(bool bLista1, Tabela* tabela, const Glib::ustring &sCaminho) {
	Glib::ustring sPaiCaminho = "";
...
void Catalogador::TamTotalDiretorio(const Glib::ustring &sCaminho) {
	long conta;
	int nAba;
...
std::vector<Glib::ustring> Catalogador::montaCaminho(const Glib::ustring &sCaminho, bool bRemover) {
	//sCaminho = NO_RAIZ + Rotinas.BARRA_INVERTIDA + sCaminho;
	std::vector<Glib::ustring> sl = Rotinas::SListSeparador(sCaminho, Rotinas::BARRA_INVERTIDA[0]);
	guint tam = sl.size();
	Glib::ustring item = sl.at(tam - 1);
...
std::vector<Glib::ustring> Catalogador::montaCaminhoStatus(Arvore* arvore, Gtk::TreeIter iter) {
...
void Catalogador::Pesquisar() {
	...
	if (arvore->getStore()->children().size() > 0) {
		...
		Glib::ustring texto = frmPrincipal->edtPesquisa->get_text();
...
void Catalogador::PesquisarArvoreDiretorio(const Glib::ustring &sCaminho, Aba aba) {
	frmPrincipal->notebook1->set_current_page(aba.getOrdem() - 1);

	Arvore* arvore = getArvoreAtual();

	std::vector<Glib::ustring> sl = montaCaminho(sCaminho, false);
...
void Catalogador::LerArvoreDiretorio(Arvore* arvore, 
	...
	std::vector<Glib::ustring> lista = montaCaminhoStatus(arvore, iter);
...
void Catalogador::DuploCliqueLista(const Glib::ustring &nome, const Glib::ustring &tipo) {
	if (tipo == Rotinas::getRecurso("TipoLista.diretorio")) {
		frmPrincipal->get_window()->set_cursor(Gdk::Cursor(Gdk::WATCH));

		Glib::ustring sCaminho = frmPrincipal->getLabStatus2()->get_text();
...
void Catalogador::EncontrarItemLista(const Glib::ustring &nomeAba, const Glib::ustring &nome,
...
int Catalogador::verificaNomeDiretorio(const Glib::ustring &sCaminho, RotuloRaiz *sRotuloRaiz) {
...
void Catalogador::ExportarArquivo(TipoExportar tipo, IProgressoLog pLog) {
	...
	Aba aba = getAbaAtual();
	Glib::ustring sDiretorio = Rotinas::AppDirPath();
	Glib::ustring sArquivo = aba.getNome() + "." + sExtensao;
	Glib::ustring arquivo = EscolhaArquivo::salvarArquivo2(frmPrincipal,
	...
	if (iter)
	{
		if (arvore->get_selection()->is_selected(iter)) {
			...
			if (res) {
				...
				std::vector<Glib::ustring> sl = montaCaminhoStatus(arvore, iter);
				Glib::ustring caminho = Rotinas::SListTexto(sl);
...
void Catalogador::ImportarArquivo(std::vector<Glib::ustring> log, IProgressoLog pLog) {
	int nResultado;

	Glib::ustring arquivo = EscolhaArquivo::abrirArquivo(frmPrincipal, EscolhaArquivo::FILTRO_XML);
...
bool Catalogador::ImportarDiretorios(std::vector<Glib::ustring> listaCaminho,
	...
	Glib::ustring sCaminhoSemDrive, sCaminho = "";
	Importar importar;
...
void Catalogador::ComecaImportacao(bool bSubDiretorios, IProgressoLog pLog) {
	...
	std::vector<Glib::ustring> listaCaminho;
