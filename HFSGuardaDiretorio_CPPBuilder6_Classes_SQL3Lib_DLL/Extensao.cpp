...
void TExtensao::carregarExtensao(std::vector<TExtensao> *listaExtensao,
	...
	*/
	dm->tabelaExtensoes = dm->conexao->GetTable(dm->SQLExtensoes.c_str());
	...
	if (total > 0) {
		...
		for (int i = 0; i < total; i++) {
			...
			*/
			 ...
			 extensao.codigo = dm->tabelaExtensoes->FieldAsInteger(
				dm->tabelaExtensoes->FieldIndex("cod"));
			 extensao.nome = dm->tabelaExtensoes->FieldAsString(
				dm->tabelaExtensoes->FieldIndex("nome")).c_str();
...
// ---------------------------------------------------------------------------
 void TExtensao::incluirExtensao(TExtensao extensao) {
	...
	try {
		...
		ms->Position = 0;
		sSQL = "UPDATE Extensoes set bmp16=? WHERE cod=" +
			IntToStr(extensao.codigo);
		dm->conexao->UpdateBlob((char*)sSQL.c_str(), ms);
	}
	__finally {
		ms->Free();
	}
	...
	try {
		...
		ms->Position = 0;
		sSQL = "UPDATE Extensoes set bmp32=? WHERE cod=" +
			IntToStr(extensao.codigo);
		dm->conexao->UpdateBlob((char*)sSQL.c_str(), ms);
	}
	__finally {
		...
		if (Diretorio->carregarArquivos(caminho, listaArquivos)) {
			...
			for (int i = 0; i < listaArquivos->Count; i++) {
				...
				sExtensao =
					...
					1, sArquivo.Length());
				sExtensao = sExtensao.SubString(1, sExtensao.Pos(".") - 1);
