...
*/
void TDiretorio::carregarDiretorio(AnsiString sSQL,
	std::vector<TDiretorio> *listaDiretorio, TProgressoLog progressoLog) {
	TDiretorio diretorio;
	int total;
	TSQLiteTable *consultaDir;
	String sTamanho;
	double nTamanho;
	TProgresso pb;
	...
	consultaDir = dm->conexao->GetTable(sSQL.c_str());
	...
	if (total > 0) {
		...
		for (int i = 0; i < total; i++) {
			...
			diretorio.aba.codigo = consultaDir->FieldAsInteger(
				consultaDir->FieldIndex("aba"));
			diretorio.codigo = consultaDir->FieldAsInteger
				(consultaDir->FieldIndex("cod"));
			diretorio.ordem = consultaDir->FieldAsInteger
				(consultaDir->FieldIndex("ordem"));

			if (!consultaDir->FieldIsNull(consultaDir->FieldIndex("coddirpai")))
				diretorio.codDirPai = consultaDir->FieldAsInteger
				(consultaDir->FieldIndex("coddirpai"));
			else
				diretorio.codDirPai = -1;

			diretorio.nome =
				consultaDir->FieldAsString(consultaDir->FieldIndex("nome"))
				.c_str();

			if (!consultaDir->FieldIsNull(consultaDir->FieldIndex("tam"))) {
				nTamanho = consultaDir->FieldAsDouble
					(consultaDir->FieldIndex("tam"));
				sTamanho = FloatToStr(nTamanho);
				if (sTamanho.Pos(".") > 0)
					sTamanho = StringReplace(sTamanho, ".0", "",
					TReplaceFlags() << rfReplaceAll << rfIgnoreCase);
				diretorio.tamanho = StrToInt64(sTamanho);
			} else
                diretorio.tamanho = 0;

			diretorio.tipo.codigo =
				consultaDir->FieldAsString(consultaDir->FieldIndex("tipo"))
				.c_str()[0];
			diretorio.modificado =
				StrToDateTime(consultaDir->FieldAsString
				(consultaDir->FieldIndex("modificado")).c_str());
			diretorio.atributos =
				consultaDir->FieldAsString(consultaDir->FieldIndex("atributos"))
				.c_str();
			diretorio.caminho =
				consultaDir->FieldAsString(consultaDir->FieldIndex("caminho"))
				.c_str();
			diretorio.aba.nome =
				consultaDir->FieldAsString(consultaDir->FieldIndex("nomeaba"))
				.c_str();
			diretorio.nomePai =
				consultaDir->FieldAsString(consultaDir->FieldIndex("nomepai"))
				.c_str();
			diretorio.caminhoPai =
				consultaDir->FieldAsString
				(consultaDir->FieldIndex("caminhopai")).c_str();
			diretorio.tamanhoFormatado = MontaTamanho(diretorio.tamanho);
			if (diretorio.tipo.codigo == 'D')
				diretorio.tipo.nome = "Diretório";
			else
...
TDiretorio TDiretorio::atribuiDiretorio(int nAba, int nCodDirRaiz,
	...
	if ((sCaminho.Length() > 0) && (sNomeDirRaiz.Length() > 0)) {
		if (AnsiStartsStr(sNomeDirRaiz, sCaminho)) {
                        sCaminho = sCaminho.SubString(
                           sNomeDirRaiz.Length()+2, sCaminho.Length());
		}
		else {
...
void TDiretorio::exportarDiretorio(TTipoExportar tipo, TAba aba,
	...
	String sTexto, sCR, sTAB, sTAB2, sSQL;
	TAba abaLocal;
	TDiretorio diretorio;
	...
	sSQL = SQL_CONSULTA_ARQUIVO + " where aba=" + IntToStr(aba.codigo) +
		" order by 1, 2, 3";
...
int TDiretorio::XMLParaDiretorio(String sTexto, int nArquivo,
	...
	if (sTexto.Pos(">") > 0)
		sTagInicio = sTexto.SubString(1, sTexto.Pos(">"));
	if ((sTexto.Pos(">") > 0) && (sTexto.Pos("</") > 1))
		sValor = sTexto.SubString(sTexto.Pos(">") + 1,
				sTexto.Pos("</") - sTexto.Pos(">") - 1);
