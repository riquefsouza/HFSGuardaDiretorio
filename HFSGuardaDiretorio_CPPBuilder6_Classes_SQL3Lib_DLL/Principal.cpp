...
TTreeNode* TFrmPrincipal::PesquisarArvoreDiretorio(TTreeView *tvLocal,
	...
	slTemp->Text = StringReplace(sCaminhoPai, BARRA_INVERTIDA,
		char(13)+char(10), TReplaceFlags() << rfReplaceAll << rfIgnoreCase);
...
void TFrmPrincipal::EncontrarItemLista() {
	...
	if (lvPesquisa->SelCount > 0) {
		...
		if (ti != NULL) {
			...
			nAltura =
				Math::RoundTo
				(abs(lvLocal->Font->Height * lvLocal->Font->PixelsPerInch) / 72,
				20) + 2;
...
int TFrmPrincipal::verificaNomeDiretorio(String sCaminho, String* sRotuloRaiz) {
	...
	else if ((*sRotuloRaiz).Pos(BARRA_INVERTIDA) > 0) {
...
void __fastcall TFrmPrincipal::lvFixaColumnClick(TObject *Sender,
      TListColumn *Column)
{
...
void __fastcall TFrmPrincipal::lvFixaCompare(TObject *Sender,
...
void __fastcall TFrmPrincipal::lvPesquisaColumnClick(TObject *Sender,
      TListColumn *Column)
{
  listaColunaClick(Sender, Column);
}
...
void __fastcall TFrmPrincipal::lvPesquisaCompare(TObject *Sender,
      TListItem *Item1, TListItem *Item2, int Data, int &Compare)
{
  listaCompara(Sender, Item1, Item2, Data, Compare);
}
...
void __fastcall TFrmPrincipal::tvFixaCollapsed(TObject *Sender,
      TTreeNode *Node)
{
  Node->ImageIndex = 0;
}
...
void __fastcall TFrmPrincipal::tvFixaExpanded(TObject *Sender,
      TTreeNode *Node)
{
  Node->ImageIndex = 1;
}
