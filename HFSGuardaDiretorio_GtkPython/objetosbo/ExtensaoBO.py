...
class ExtensaoBO(object):
	...
	def __init__(self):
		...
		self._diretorioBMP = Rotinas.LerArquivoImagem("diretorio.bmp", True)
		self._arquivoBMP = Rotinas.LerArquivoImagem("arquivo.bmp", True)
		self._DiretorioGIF = Rotinas.LerArquivoPixbuf("diretorio.gif")
		self._DirAbertoGIF = Rotinas.LerArquivoPixbuf("dir-aberto.gif")
		self._CDOuroGIF = Rotinas.LerArquivoPixbuf("cdouro.gif")
	...
	def indiceExtensao(self, lista, nomeExtensao):
		...
		while enumerator.MoveNext():
			...
			if extensao.Nome.Trim().ToLower().Equals(nomeExtensao.Trim().ToLower()):
				return extensao.Ordem
		return -1

	def pixbufExtensao(self, lista, nomeExtensao):
		nomeExtensao = Rotinas.SubString(nomeExtensao, Rotinas.LastDelimiter(".", nomeExtensao) + 1, nomeExtensao.Length)
		enumerator = lista.GetEnumerator()
		while enumerator.MoveNext():
			extensao = enumerator.Current
			if extensao.Nome.Trim().ToLower().Equals(nomeExtensao.Trim().ToLower()):
				arquivoGIF = Rotinas.BmpParaImagem(extensao.Bmp16, Rotinas.EXTENSAO_GIF)
	...
	def ExportarExtensao(self, tipo, listaExtensao):
		if listaExtensao.Count > 0:
			sCaminho = Rotinas.ExtractFilePath(Rotinas.ExecutablePath()) + Rotinas.BARRA_INVERTIDA
