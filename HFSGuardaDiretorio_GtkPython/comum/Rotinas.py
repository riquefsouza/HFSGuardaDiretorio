...
class Rotinas(object):
	...
	def LerArquivoImagem(sCaminho, bCaminhoPadrao):
		if bCaminhoPadrao:
	...
	ExecutablePath = staticmethod(ExecutablePath)

	def calculaProgresso(nMaximo, nProgresso):
		nMaximo += 1
		nProgresso += 1
	...
	calculaProgresso = staticmethod(calculaProgresso)

	def byteArrayToPixbuf(byteArrayIn):
		pxb = Gdk.Pixbuf(byteArrayIn)
		return pxb

	byteArrayToPixbuf = staticmethod(byteArrayToPixbuf)

	def LerArquivoPixbuf(sCaminho):
		sCaminho = Rotinas.ExtractFilePath(Rotinas.ExecutablePath()) + Path.DirectorySeparatorChar + "imagens" + Path.DirectorySeparatorChar + sCaminho
		pxb = Gdk.Pixbuf(sCaminho)
		return pxb

	LerArquivoPixbuf = staticmethod(LerArquivoPixbuf)